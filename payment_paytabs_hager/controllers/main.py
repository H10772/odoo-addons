...
class PayTabsController(http.Controller):
    _return_url = '/payment/paytabs_hager/return'
    _callback_url = '/payment/paytabs_hager/callback'
    ...
    def paytabs_callback(self, **data):
        ...
        if data:
            request.env['payment.transaction'].sudo()._handle_notification_data('paytabs_hager', data)
    ...
    def _find_transaction(self, cart_id, tran_ref):
        ...
        if cart_id:
            tx_sudo = request.env['payment.transaction'].sudo().search(
                [('reference', '=', cart_id), ('provider_code', '=', 'paytabs_hager')], limit=1
            )
        if not tx_sudo and tran_ref:
            tx_sudo = request.env['payment.transaction'].sudo().search(
                [('provider_reference', '=', tran_ref), ('provider_code', '=', 'paytabs_hager')], limit=1
            )
        return tx_sudo
